...
%             vd_foot(1:3) = zeros(3, 1);
            ...
            [p, v] = calculate_climbing_trajectory2(t, phase_start, pd_foot(1:3), ...
    ...
    if get_next_step_start2(p_foot_initial(10:12)) < start_distance
        ...
        [p, v] = calculate_walking_trajectory2(t, phase_start, pd_foot(10:12), ...
            p_foot_initial(10:12), pdelta_foot(10:12), vd_foot(10:12), tswing, 1);
        pd_foot(10:12) = p;
        vd_foot(10:12) = v;
    end        
else
...
%               vd_foot(4:6) = zeros(3, 1);
            ...
            [p, v] = calculate_climbing_trajectory2(t, phase_start, pd_foot(4:6), ...
    ...
    if get_next_step_start2(p_foot_initial(7:9)) < start_distance
        ...
        [p, v] = calculate_walking_trajectory2(t, phase_start, pd_foot(7:9), ...
            p_foot_initial(7:9), pdelta_foot(7:9), vd_foot(7:9), tswing, 1);
        pd_foot(7:9) = p;
        vd_foot(7:9) = v;
    end
end
...
function [pd_foot, vd_foot] = calculate_walking_trajectory2(t, phase_start, ...
    pd_foot, p_foot_initial, pdelta_foot, vd_foot, tswing, is_rear_leg)
    t = t - phase_start;
    h = 0.05;

    if is_rear_leg && p_foot_initial(3) < 0.1
        pdelta_foot = pdelta_foot + 0.1;
    end
    pdelta_foot = max(pdelta_foot, 0);
    pd_foot(1) = p_foot_initial(1) + pdelta_foot(1)*(t/tswing - 0.5 / pi * sin(2*pi*t/tswing));
    vd_foot(1) = pdelta_foot(1)*(1/tswing - 1 / tswing * cos(2*pi*t/tswing));
    
    if t < tswing/2
        pd_foot(3) = p_foot_initial(3) + 2*h*(t/tswing - 0.25 / pi * sin(4*pi*t/tswing));
        vd_foot(3) = 2*h*(1/tswing - 1 / tswing * cos(4*pi*t/tswing));
    else
        pd_foot(3) = p_foot_initial(3) + 2*h*(1 - t/tswing + 0.25 / pi * sin(4*pi*t/tswing));
        vd_foot(3) = 2*h*(-1/tswing + 1 / tswing * cos(4*pi*t/tswing));
    end
end
...
function [pd_foot, vd_foot] = calculate_climbing_trajectory2(t, phase_start, ...
    pd_foot, p_foot, vd_foot, p_foot_initial, tswing)
